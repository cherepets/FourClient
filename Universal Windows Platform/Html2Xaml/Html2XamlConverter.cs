...
namespace Html2Xaml
{
    ...
    public static class Html2XamlConverter
	{
        ...
        private static void parseImage(StringBuilder xamlString, HtmlNode imageNode)
        {
            var src = imageNode.Descendants("src").FirstOrDefault();
            if (src != null)
            {
                xamlString.Append("<InlineUIContainer>");
                string xaml = $"<Image Height='50' Width='50' Source=\"{src.InnerText}\" />";
                xamlString.Append(xaml);
                xamlString.Append("</InlineUIContainer>");
            };
        }
