...
public class HBR_GetPage : ITransformation
{
    const String Url = "http://habrahabr.ru/";
    ...
    private Dictionary<string, string> ParseDiv(XElement xdiv)
    {
        try
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                {"Link", link},
                {"Image", image},
                {"FullLink", "http://m.habrahabr.ru/post/" + link},
                {"CommentLink", "http://m.habrahabr.ru/post/" + link + "/comments"},
            };
        }
        catch (Exception ex)
        {
            var exceptionBuilder = new StringBuilder();
            exceptionBuilder.AppendLine("Method: " + "HBR_GetPage/ParseDiv");
