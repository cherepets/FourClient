...
public class W7P_GetPage : ITransformation
{
    const String Url = "http://w7phone.ru/";
        ...
        switch (newsType)
        {
            ...
            default:
            {
                ...
                var filtered = divs.Where(d => d
                    .AttributeValue("class") == "entry-thumb")
                    .ToList();      
                var list = filtered
                    .Select(d => ParseDiv(d))
                    .ToList();
    ...
    private Dictionary<string, string> ParseArticle(XElement xdiv)
    {
        try
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                {"Link", link},
                {"Image", String.Empty},
                {"FullLink", "http://w7phone.ru/" + link},
                {"CommentLink", "http://w7phone.ru/" + link + "#disqus_thread"},
            };
        }
        catch (Exception ex)
        {
