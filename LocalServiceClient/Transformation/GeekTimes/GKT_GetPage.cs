...
public class GKT_GetPage : ITransformation
{
    const String Url = "http://geektimes.ru/";
    ...
    private Dictionary<string, string> ParseDiv(XElement xdiv)
    {
        try
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                {"Link", link},
                {"Image", image},
                {"FullLink", "http://m.geektimes.ru/post/" + link},
                {"CommentLink", "http://m.geektimes.ru/post/" + link + "/comments"},
            };
        }
        catch (Exception ex)
        {
            var exceptionBuilder = new StringBuilder();
            exceptionBuilder.AppendLine("Method: " + "GKT_GetPage/ParseDiv");
